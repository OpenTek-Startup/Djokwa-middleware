...
export const createExpense = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (!budget) {
      res.status(400).json({ error: "A valid budget must be created before adding an expense." });
      return;
    }
...
export const generateExpenseReport = async (req: Request, res: Response): Promise<void> => {
