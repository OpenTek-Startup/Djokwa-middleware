...
const validateDates = (startDate: any, endDate: any) => {
  if (startDate && isNaN(Date.parse(startDate))) throw new Error('Invalid startDate format');
  if (endDate && isNaN(Date.parse(endDate))) throw new Error('Invalid endDate format');
...
export const exportReportToExcel = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    donations.forEach((donation) =>
      sheet.addRow({ type: 'Donation', amount: donation.Amount, date: donation.DonationDate })
    );
    expenses.forEach((expense) =>
      sheet.addRow({ type: 'Expense', amount: expense.Amount, date: expense.Date })
    );
