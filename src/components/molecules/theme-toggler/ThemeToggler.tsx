...
export default function ThemeToggler({ className, containerClassName }: Props) {
  const isDarkTheme =
  ...
  return (
    <>
      ...
      <div
        ...
        className={cn(
          'w-[min(8rem,calc(100%-1rem))] bg-white ring-1 cursor-pointer py-1.5 px-4  rounded-full flex items-center mx-auto ',
      ...
      >
        ...
        >
          <div className="size-full  flex items-center justify-center rounded-full">
            {isDarkTheme ? (
              <IconRepository.SunIcon width={20} height={20} />
            ) : (
              <CiSun size={20} />
            )}
          </div>
